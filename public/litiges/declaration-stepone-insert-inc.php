...
if(count($errors)==0){
	...
	for ($i=0; $i <count($_POST['article_id']) ; $i++){
		...
		$insertDetail=$litigeDao->addDetails($lastInsertId,$numDossier, $palette, $fac, $dateFact, $art, $ean,$dossier, $descr, $qte,$tarif, $fou, $cnuf,$tete,$detailbox,$occArticlePalette, $puv,$pul );
	...
	if($added==count($_POST['article_id']) && !isset($_POST['palette_complete'])){
		header('Location:declaration-steptwo.php?id='.$lastInsertId);
	}elseif ($added==count($_POST['article_id']) && isset($_POST['palette_complete'])){
