...
function getAction($pdoLitige){
	$req=$pdoLitige->prepare("SELECT id, libelle,id_dossier, action.id_web_user, DATE_FORMAT(date_action, '%d-%m-%Y')as dateFr, pj, achats FROM action WHERE id_dossier= :id_dossier AND (id_contrainte=12 OR id_contrainte=13) ORDER BY date_action");
	$req->execute(array(
		':id_dossier'		=>$_GET['id']
...
function getListDossier($pdoLitige){
	$req=$pdoLitige->query("SELECT DISTINCT dossier,dossiers.id  FROM action LEFT JOIN dossiers ON action.id_dossier=dossiers.id WHERE id_contrainte=12 OR id_contrainte=13 ORDER BY dossier DESC");
	$req->execute();
	return $req->fetchAll(PDO::FETCH_ASSOC);
}
...
function addAction($pdoLitige, $filelist){
	...
	$msg=nl2br($msg);
	$req=$pdoLitige->prepare("INSERT INTO action (id_dossier,libelle,id_contrainte,id_web_user,commission_sav,pj, date_action) VALUES (:id_dossier,:libelle,:id_contrainte,:id_web_user,:commission_sav,:pj,:date_action)");
	$req->execute(array(
		':id_dossier'		=>	$_GET['id'],
		':libelle'		=>	$msg,
		':id_contrainte'	=>13,
...
// list déroulante avec dossiers
if(isset($_POST['id_dossier'])){
	header('Location:intervention-commission-sav.php?id='.$_POST['id_dossier']);
}
...
// vérifie si pièce jointes
	...
	if(count($errors)==0)
	{
		...
		// envoi mail litigelivraison
			...
			$subject='Portail BTLec - litige - Réponse commission SAV '.$thisLitige[0]['dossier'];
...
<?php if (isset($_GET['id']) && isset($thisLitige[0]['dossier'])): ?>
	...
	<div class="row mb-3 pt-3">
		<div class="col heavy text-main-blue">Echanges avec le magasin :</div>
	</div>
	<?php
	...
	foreach ($dials as $dial)
	{
		...
		if(!empty($dial['msg']))
		{
			if($dial['mag']==1 ||$dial['mag']==3){
					$from=$thisLitige[0]['mag'];
			}else{
				$from=UserHelpers::getFullname($pdoUser, $dial['id_web_user']);
			}
			if($dial['filename']!='')
			{
	...
	// conteneur
			echo '<div class="row alert '.$repColor[$dial['mag']].' mb-5">';
			echo '<div class="col">';
// ligne 1
			...
			echo '<div class="col">';
			echo '<div class="text-right"><i class="far fa-calendar-alt pr-3"></i>'.$dial['dateFr'].'</div>';
			echo '</div>';
...
// ligne 2
	...
	<div class="row mb-3 py-3 ">
		<div class="col">
			<div class="text-main-blue heavy">Répondre au service litige livraison:</div>
		</div>
	</div>
	<div class="row pb-5">
		<div class="col-2"></div>
