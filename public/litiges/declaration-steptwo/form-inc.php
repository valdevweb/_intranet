<div class="row">
		...
		<?php foreach ($fLitige as $keyLitige => $litige): ?>
			<?php if($litige['box_tete']!=1):?>
				...
				<div class="row border pt-3 mb-5">
						...
						<div class="row">
							<div class="col-4">
								...
								<div class="form-group">
									<select class="form-control" name="form_motif[<?=$keyLitige?>]"  id="motif<?=$litige['detail_id']?>"required data-gt="<?=$litige['gt']?>">
					...
					<div class="hidden" id="toggleSerial<?=$litige['detail_id']?>">
						<div class="row mt-3">
							<div class="col-12  pl-3">
								<div class="khand heavy">Numéros de série des produits :</div>
								<div class="text-danger">Merci de séparer les numéros de série par une virgule</div>
								<div class="form-group">
									<textarea class="form-control" name="serials[<?=$keyLitige?>]"  row="3" id="textarea<?=$litige['detail_id']?>"></textarea>
								</div>
							</div>
						</div>
					</div>
	...
	<div class="form-group">
		<textarea class="form-control cmt" name="form_com"><?= isset($cmt)? str_replace('<br />', "\n",'Demande d\'origine du magasin : <br />' .$cmt):''?></textarea>
	</div>
</div>
</div>
