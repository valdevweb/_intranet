...
foreach ($_POST['form_id'] as $postKey => $idDetail) {
	...
	if(empty($errors)){
		...
		if(isset($_POST['serials'][$postKey])){
			$serials=$_POST['serials'][$postKey];
		}
		if(!isset($_POST['radio-inv'][$postKey]) || $_POST['radio-inv'][$postKey]!=1){
			...
			$ean="";
			$do=updateDetail($pdoLitige,$_POST['form_motif'][$postKey], $_POST['form_qte'][$postKey],$_POST['form_id'][$postKey],$allfilename,$ean, $valoLig, $serials);
		}else{
			// inversion de ref
