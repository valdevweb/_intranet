...
$thisCorres=$listCorresp[$_GET['id_contrainte']];
...
if (isset($_GET['id'])) {
	$paramContrainte = '(id_contrainte=5 OR id_contrainte=4)';
...
if (isset($_POST['submit'])) {
	...
	if (count($errors) == 0) {
		$add = $actionDao->addActionLitigeService($_GET['id'], $_POST['msg'], 5, $allfilename, ['field'=>'sav', 'value'=>1]);
			...
			$htmlMail = file_get_contents('mail/mail_rep_sav.php');
			...
			$successQ='?success=send&id='.$_GET['id'];
			unset($_POST);
...
<div class="container">
	...
	<?php if (isset($_GET['id']) && !empty($thisLitige)) : ?>
		...
		<div class="row pb-3">
			<div class="col"><span class="heavy text-yellow-dark"> Magasin : </span><?= $thisLitige[0]['deno'] . ' - ' . $thisLitige[0]['btlec'] ?></div>
			<div class="col"><span class="heavy text-yellow-dark"> Centrale : </span><?= $listCentrales[$thisLitige[0]['centrale']] ?></div>
			...
			<div class="col-2 text-right"><i class="fas fa-calendar-alt pr-3 text-yellow-dark"></i><?= date('d-m-Y', strtotime($thisLitige[0]['date_crea'])) ?></div>
		</div>
		<div class="row">
			<div class="col">
