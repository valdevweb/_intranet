...
function isOneRightInArray($arrayRights){
	for ($i=0; $i <count($arrayRights) ; $i++) {
		if (in_array($arrayRights[$i], $_SESSION['id_droit'])) {
			echo $arrayRights[$i];
			return true;
			// arrivé au dernier droit, si c'est encore faux, on renvoie faux
		}elseif(!in_array($arrayRights[count($arrayRights)-1], $_SESSION['id_droit'])){
			return false;
		}
	}
}
...
$revIds=array(5,7,8);
$d_rev=isOneRightInArray($revIds);
// accès comm : admin, comm
$comIds=array(5,6);
$d_comm=isOneRightInArray($comIds);

$btlecIds=array(4);
$d_btlec=isOneRightInArray($btlecIds);


// accès exploit + : admin
$exploitIds=array(5);
$d_exploit=isOneRightInArray($exploitIds);

//accès restreint exploit (dsy, cde, luc, moi)
$strictAdminIds=array(82);
$d_strictAdmin=isOneRightInArray($strictAdminIds);

//accès restreint exploit (dsy, cde, luc, moi)
$restraintAdminIds=array(87);
$d_restraintAdmin=isOneRightInArray($restraintAdminIds);

// accès exploit stat : admin + chargé de mission
$exploitStatIds=array(5,78);
$d_exploitStat=isOneRightInArray($exploitStatIds);

$searchMagIds=array(5,78);
$d_searchMag=isOneRightInArray( $searchMagIds);
// accès conseil : admin, consultation conseil,exploit conseil, inscription
$conseilIds=array(5,9,10,27);
$d_conseil=isOneRightInArray($conseilIds);
// accès contactez nos services : compte mag
$magIds=array(2);
$d_mag=isOneRightInArray($magIds);
//accès lcom user (lecture uniquement)
$lcomUserIds=array(43,44);
$d_lcomUser=isOneRightInArray($lcomUserIds);
//accès lcom admin : luc et amenet
$lcomAdminIds=array(44);
$d_lcomAdmin=isOneRightInArray($lcomAdminIds);

$tempSavIds=array(62);
$d_tempSav=isOneRightInArray($tempSavIds);

$magSocamilIds=array(66,5);
$d_Socamil=isOneRightInArray($magSocamilIds);

$litigeBtIds=array(29,69,78,7,79,80);
$d_litigeBt=isOneRightInArray($litigeBtIds);
	echo "<pre>";
	print_r($d_litigeBt);
	echo '</pre>';

$missionIds=array(78,5);
$d_mission=isOneRightInArray( $missionIds);

$gtOccBtIds=array(83);
$d_occBt=isOneRightInArray($gtOccBtIds);

$gtOccMagIds=array(84);
$d_occMag=isOneRightInArray($gtOccMagIds);
