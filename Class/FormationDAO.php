...
class FormationDAO{
	...
	public function getIncriptionByFormation($pdoBt, $idFormation){
		$req=$this->pdoBt->prepare("SELECT * FROM salon_formations LEFT JOIN salon_creneaux ON id_creneau = salon_creneaux.id WHERE id_formation= :id_formation ORDER BY creneaux.start");
		$req->execute([
	...
	public function addFormation($pdoBt, $idFormation){
		...
		$req=$this->pdoBt->prepare("INSERT INTO salon_formations (id_formation, id_web_user, mardi, mercredi, id_creneau_1, id_creneau_2, nb, email)VALUES(:id_formation, :id_web_user, :mardi, :mercredi, :id_creneau_1, :id_creneau_2, :nb, :email)");
		$req->execute([
			':id_formation'	=>$idFormation,
