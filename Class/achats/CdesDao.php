...
class CdesDao
{
	...
	public function getCdesOpRelances($dateStart, $dateEnd, $param = null)
	{
		...
		$req = $this->pdo->prepare("
			SELECT cdes_encours.*,{$this->version}doc_achats.cdes_cmts.cmt_btlec, {$this->version}doc_achats.cdes_cmts.cmt_galec  
			FROM cdes_encours left join {$this->version}doc_achats.cdes_cmts ON cdes_encours.id={$this->version}doc_achats.cdes_cmts.id
			WHERE date_cde IS NOT NULL AND date_cde < :oneweekold AND qte_cde !=0 AND date_start BETWEEN :date_start AND :date_end $param ORDER BY fournisseur, date_cde");
		$req->execute([
	...
	public function getCdesPermRelances($param)
	{
		...
		SELECT cdes_encours.*,{$this->version}doc_achats.cdes_cmts.cmt_btlec, {$this->version}doc_achats.cdes_cmts.cmt_galec  
			FROM cdes_encours left join {$this->version}doc_achats.cdes_cmts ON cdes_encours.id={$this->version}doc_achats.cdes_cmts.id
			 WHERE date_cde IS NOT NULL AND date_cde < :oneweekold AND dossier=1000 AND qte_cde !=0 $param ORDER BY fournisseur, date_cde");
		$req->execute([
			':oneweekold'		=> $oneweekOld->format('Y-m-d'),
