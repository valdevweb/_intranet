...
class Select{
	...
	private $id;			//par défaut value= les valeurs de id du tableau de résultat arrData
	// private $texts;
	...
	public function createOption($arrayData,$field,$id=null)
	{
		$selected='';
		...
		if($id==null)
		{
			foreach ($arrayData as $data)
			{
				...
				{
					if($_POST[$this->field=$field]==$data['id'])
					{
						$selected='selected';
					}
					else
				...
				// echo '<option value="'.$this->id=$data[$this->id=$id].'">'.$this->texts=$data[$this->field=$field].'</option>';
			}
		...
		else
		{
			foreach ($arrayData as $data)
			{
				if(!empty($_POST[$this->field=$field]))
				{
					...
					if($_POST[$this->field=$field]==$data[$this->id=$id])
					{
						$selected='selected';
					}
					else{
				...
				// echo '<option value="'.$data['id'].'" '.$selected.'>'.$data[$this->field=$field].'</option>';
					echo '<option value="'.$data[$this->id=$id].'" '.$selected.'>'.$data[$this->field=$field].'</option>';
